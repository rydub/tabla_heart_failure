...
def get_features(patient_dir):
    """Calculate the mean mfcc from coefficients 1-12 for each PS recording (3 per region)"""

    ps_recordings = [file_ for file_ in os.listdir("%s/PS" % patient_dir) if file_.endswith(".wav")]
    ps_features = []
    ps_headers = []
    ...
    # bs_recordings = [file_ for file_ in os.listdir("%s/BS" % patient_dir) if file_.endswith(".wav")]
    # bs_features = []
    # bs_headers = []
    ...
    return ps_headers, average_ps_features
