...
def get_visit_dict(pat_idx):
    """returns a dict keyed by patient id containing a tuple of lists ([dated record ids], [days since admission])"""
...
def get_fluids(record_id):
    """return the fluid overload in mL for the corresponding record id"""
    df = _get_patient_recordings()
    return df.loc[record, 'Fluid Up (mL)']
...
def _get_patient_recordings():
    return pd.read_csv('features/recordings_HF.csv', index_col=0, usecols=range(9))


def _get_patient_features():
    """returns a features df, merges in date columns for utility"""
    recordings = _get_patient_recordings()
...
visit_dict = get_visit_dict(features, pat_full_idx)
for pat in pat_full_idx:
    y_vals = []
    for record in _extract_record_names(features, pat):
        y_vals.append(get_fluids(record, recordings))
